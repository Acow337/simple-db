...
public class LogTest extends SimpleDbTestBase {
    ...
    public void TestOpenCommitCheckpointOpenCrash()
        ...
        // *** Test:
        // T1 inserts but does not commit   value: 12 13
        // T2 inserts and commits   value: 26 27 commit
        // checkpoint
        // T3 inserts but does not commit value: 28 29
        // crash
        // only T2 data should be there

        Transaction t1 = new Transaction();
        t1.start();
        System.out.println("LogTest: T1 start: " + t1.getId().getId());
        insertRow(hf1, t1, 12);
        Database.getBufferPool().flushAllPages(); // XXX defeat NO-STEAL-based abort
        insertRow(hf1, t1, 13);
        ...
        Transaction t3 = new Transaction();
        System.out.println("LogTest: T3 start: " + t3.getId().getId());
        t3.start();
        insertRow(hf2, t3, 28);
        Database.getBufferPool().flushAllPages(); // XXX defeat NO-STEAL-based abort
        insertRow(hf2, t3, 29);
